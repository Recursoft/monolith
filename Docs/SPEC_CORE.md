...
| **Total** | | **1290** registrations across 16 namespaces + 4 `monolith_*` meta = **1286 in-tree namespace actions** active by default (45 town-gen experimental disabled). With town gen registered: 1335. Without `WITH_COMMONUI`: 1235 (and 1280 with town gen); without `WITH_GBA`: 1155 (1200 w/ town gen); without both: 1104 (1149 w/ town gen). Without `WITH_METASOUND`: 1274 (the v0.14.10 12 introspection actions become inert) | The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1282** in default-active configuration, **1331** with town gen registered |
