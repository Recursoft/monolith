...
| MonolithAnimation | animation | 120 | Includes 5 ABP write actions (`add_anim_graph_node`, `connect_anim_graph_pins`, `set_state_animation`, `add_variable_get`, `set_anim_graph_node_property`), 3 ControlRig write, 1 layout, plus 98 baseline (96 + 1 v0.14.9 `copy_bone_pose_between_sequences` — PR #51 by @MaxenceEpitech + 1 v0.14.10 `list_bone_tracks` — PR #54 by @MaxenceEpitech) + 13 PoseSearch |
...
| **Total** | | **1294** registrations across 16 namespaces + 4 `monolith_*` meta = **1290 in-tree namespace actions** active by default (45 town-gen experimental disabled). With town gen registered: 1339. Without `WITH_COMMONUI`: 1239 (and 1284 with town gen); without `WITH_GBA`: 1159 (1204 w/ town gen); without both: 1108 (1153 w/ town gen). Without `WITH_METASOUND`: 1278 (the v0.14.10 12 introspection actions become inert) | The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1286** in default-active configuration, **1335** with town gen registered |
